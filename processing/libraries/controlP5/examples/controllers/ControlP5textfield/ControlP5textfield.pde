...
void draw() {
  background(0);
  fill(255);
  text(cp5.get(Textfield.class,"input").getText(), 240,130);
  text(textValue, 200,180);
  
  loadPixelsImage = createImage(700, 400, RGB);
  //testing grabbing a portion of the interface for text
  loadPixels(); 
  // Loop through every pixel
  for (int i = 0; i < pixels.length; i++) {
    loadPixelsImage.pixels[i] = pixels[i];
  }
  image(loadPixelsImage, 0, 100);
}
